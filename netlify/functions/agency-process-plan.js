...
// FAST version: saves client + fires background, returns in <2s
// Claude call happens in agency-generate-background.js (15min timeout)
...
export default async (req) => {
  ...
  try {
    ...
    // 3. Save client immediately (status: new)
    ...
    // 4. Fire background function — does not block, background has 15min timeout
    ...
    bgReq.on('error', e => console.error('[process-plan] bg trigger error:', e.message));
    bgReq.setTimeout(3000, () => { bgReq.destroy(); });
    bgReq.write(bgPayload);
    bgReq.end();
    console.log('[process-plan] Background triggered for:', slug);

    // 5. Return immediately — plan generates in background

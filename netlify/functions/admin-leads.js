...
// Handles: add lead, list leads, delete lead, claim lead (called by agency dashboard)
...
async function fsList(collection) {
  ...
  const res = await fsHttp('GET', `${BASE()}/${collection}?pageSize=300`, null, t);
...
async function fsDelete(collection, docId) {
  const t = await getToken();
  return new Promise((resolve,reject)=>{
    const r = https.request({hostname:'firestore.googleapis.com',path:`${BASE()}/${collection}/${docId}`,method:'DELETE',headers:{'Authorization':'Bearer '+t}},res=>{res.resume();res.on('end',resolve);});
...
// Verify agency session token
async function verifyAgencySession(token) {
...
export default async (req) => {
  ...
  if (method === 'GET') {
    ...
    try {
      const leads = await fsList('lead_pool');
      // Sort newest first
      leads.sort((a,b) => new Date(b.createdAt||0) - new Date(a.createdAt||0));

      if (isAdmin) {
        // Admin sees everything including claimed
        return new Response(JSON.stringify({success:true, leads}),{status:200,headers:CORS});
      } else {
        // Agencies see unclaimed only, with contact info hidden
        const visible = leads
          .filter(l => !l.claimedBy)
          .map(l => ({
            id:           l.id,
            ...
            // Hide sensitive info until claimed
            contactName:  '••••••',
            phone:        '••• ••• ••••',
            email:        '••••@••••.com',
          }));
  ...
  if (method === 'POST') {
    ...
    if (action === 'delete') {
      ...
      await fsDelete('lead_pool', leadId);
      return new Response(JSON.stringify({success:true}),{status:200,headers:CORS});
    }

    // CLAIM LEAD — agency only
    if (action === 'claim') {
      ...
      // Get the lead
      const lead = await fsGet('lead_pool', leadId);
      ...
      // Add to agency's client list as "Paid Lead"
